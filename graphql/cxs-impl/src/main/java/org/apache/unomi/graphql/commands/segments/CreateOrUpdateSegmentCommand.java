...
public class CreateOrUpdateSegmentCommand extends BaseCommand<CDPSegment> {
    ...
    private Condition createProfileEventsCondition(final CDPProfileEventsFilterInput eventsFilterInput) {
        if (eventsFilterInput == null) {
            return null;
        }

        final CDPEventFilterInput eventFilterInput = eventsFilterInput.getEventFilter();
        ...
        if (eventsFilterInput.getNot() != null) {
            final ConditionType conditionType = serviceManager.getDefinitionsService().getConditionType("notCondition");

            subConditions.add(ConditionBuilder.builder(conditionType)
                    .setParameter("subCondition", createProfileEventsCondition(eventsFilterInput.getNot()))
                    .build());
        }

        final Condition eventCondition = ConditionBuilder.builder(booleanConditionType).buildBooleanCondition("and", subConditions);

        final ConditionType pastEventConditionType = serviceManager.getDefinitionsService().getConditionType("pastEventCondition");

        return ConditionBuilder.builder(pastEventConditionType)
