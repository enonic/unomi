...
public class CDPConsentUpdateEventInput extends BaseProfileEventProcessor {
    ...
    public CDPConsentUpdateEventInput(
            final @GraphQLName("type") String type,
    ...
    public Event buildEvent(LinkedHashMap<String, Object> eventInputAsMap, DataFetchingEnvironment environment) {
        ...
        profile.getConsents().forEach((k, v) -> {
            if (k.endsWith("/" + profilePropertiesAsMap.get("type").toString())) {
                if (profilePropertiesAsMap.get("status") != null) {
                    v.setStatus(ConsentStatus.valueOf(profilePropertiesAsMap.get("status").toString()));
                }
                if (profilePropertiesAsMap.get("lastUpdate") != null) {
                    v.setStatusDate(DateUtils.toDate((OffsetDateTime) profilePropertiesAsMap.get("lastUpdate")));
                }
                if (profilePropertiesAsMap.get("expiration") != null) {
                    v.setRevokeDate(DateUtils.toDate((OffsetDateTime) profilePropertiesAsMap.get("expiration")));
                }
            }
        });
