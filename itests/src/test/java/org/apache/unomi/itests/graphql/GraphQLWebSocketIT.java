...
public class GraphQLWebSocketIT extends BaseIT {
    ...
    public void testWebSocketConnectionSegment() throws Exception {
        ...
        try {
            ...
            String initMsg = parseJson(bundleContext.getBundle().getResource("graphql/socket/out/init.json"));
            remote.sendString(initMsg);

            String ackMsg = parseJson(bundleContext.getBundle().getResource("graphql/socket/in/ack.json"));
            String initResp = socket.waitMessage().get();
            Assert.assertEquals(ackMsg, initResp);

            String termMsg = parseJson(bundleContext.getBundle().getResource("graphql/socket/out/term.json"));
            remote.sendString(termMsg);

            CloseStatus status = socket.waitClose().get();
            Assert.assertEquals(0, (int) status.getStatus());
            Assert.assertEquals("Client terminated connection", status.getReason());
    ...
    private class Socket extends WebSocketAdapter {
        ...
        public CompletableFuture<String> waitMessage() {
        ...
        public CompletableFuture<CloseStatus> waitClose() {
            return closeStatus;
        }
